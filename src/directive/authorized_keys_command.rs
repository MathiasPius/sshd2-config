...
/// Arguments to **AuthorizedKeysCommand** accept the tokens described in the TOKENS section.
/// If no arguments are specified then the username of the target user is used.
/// The program should produce on standard output zero or more lines of authorized_keys output (see AUTHORIZED_KEYS in [sshd(8)](https://man.openbsd.org/sshd.8)).
