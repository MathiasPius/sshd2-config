...
/// Arguments to **AuthorizedPrincipalsCommand** accept the tokens described in the TOKENS section.
/// If no arguments are specified then the username of the target user is used.
/// The program should produce on standard output zero or more lines of **AuthorizedPrincipalsFile** output.
/// If either **AuthorizedPrincipalsCommand** or **AuthorizedPrincipalsFile** is specified, then certificates offered by the client for authentication must contain a principal that is listed.
...
impl<'a> AsRef<str> for AuthorizedPrincipalsCommand<'a> {
    fn as_ref(&self) -> &str {
        &self.0
    }
}

impl<'a> crate::ParseDirective<'a> for AuthorizedPrincipalsCommand<'a> {
